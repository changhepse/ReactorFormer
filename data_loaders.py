...
def load_pde(root, batch_size, num_workers=4, subset=None, size="gpt2"): 
    
    train_data = MixedDataset(root,subset, if_test=False, ratio=0.6, size=size)
    if subset is None:
        subset = "CSTR1"
...
class MixedDataset(Dataset):
    def __init__(self, saved_folder, subset=None, # subset = equation name folder
        ...
        if self.if_test:
            ...
            p = self.root_path + "/shape.npy" # [sample, t, v, x, y]
            s = np.load(p)[:2] 
            s[1] -= 1 
            if ratio < 1:
                s[0] = int(s[0] * ratio)
            ...
            train_names = ["CSTR1", "CSTR2", "BR1", "BR2", "PFR1", "PFR2"] 
            ...
            for i, name in enumerate(train_names):
                ...
                stats_path = f"{self.root_path}/{name}/stats.npy"
                stats = torch.from_numpy(np.load(stats_path))
                self.mean_dict[name] = stats[0] 
                self.std_dict[name] = stats[1]  
...
if __name__ == '__main__':
    load_pde("datasets", 32, reduced_resolution=1, prev_t=1, valid_split=-1, num_workers=0, subset=None, size="gpt2") # dataset='Burgers',
