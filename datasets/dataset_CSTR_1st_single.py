...
# 反应器方程（经典 CSTR，无空间依赖）
def cstr_reactor(t, y, F, V, CA_in, T_in, k0, Ea, H_rxn, U, rho, Cp):
    CA, T = y
    dCA_dt = (F / V) * (CA_in - CA) - k0 * np.exp(-Ea *1e3 / (R * T)) * CA
    dT_dt = (F / V) * (T_in - T) + (-H_rxn) / (rho * Cp) * k0 * np.exp(-Ea *1e3 / (R * T)) * CA + (U * A * (Tc - T)) / \
                (rho * Cp * V)
    return [dCA_dt, dT_dt]
...
file_path = os.path.join(output_dir, 'cstr_1st.h5')
# 删除已有文件
...
try:
    with h5py.File(file_path, 'w') as f:
        for i in range(num_samples):
            # 生成标量随机初始条件和参数
            F = generate_scalar_grf(sigma_F, mean_F)
            V = generate_scalar_grf(sigma_V, mean_V)
            CA_in = generate_scalar_grf(sigma_CA, mean_CA)
            T_in = generate_scalar_grf(sigma_T, mean_T)
            ...
            # 确保参数物理合理
            F = np.clip(F, 0.018, 0.022)
            V = np.clip(V, 0.08, 0.12)
            CA_in = np.clip(CA_in, 500, 1000)
            T_in = np.clip(T_in, 300, 350)
            CA0 = np.clip(CA0, 500, 1000)
            T0 = np.clip(T0, 300, 350)
            k0 = np.clip(k0, 8e8, 1.2e9)
            Ea = np.clip(Ea, 58, 62)
            ...
            # 求解（结果形状: [201, 2]）
            sol = solve_ivp(
                lambda t, y: cstr_reactor(t, y, F, V, CA_in, T_in, k0, Ea, H_rxn, U, rho, Cp),
                t_span, [CA0, T0],
            ...
            group.attrs['F'] = F
            group.attrs['V'] = V
            group.attrs['CA_in'] = CA_in
            group.attrs['T_in'] = T_in
            group.attrs['k0'] = k0
            group.attrs['Ea'] = Ea
...
# 读取 HDF5 文件
file_path = 'D:\\Python_Projects\\Reactor_Models\\cstr_1st.h5'
with h5py.File(file_path, 'r') as f:
    ...
    plt.title('CSTR 1st Concentration (CA)')
    plt.xlabel('t (s)')
    plt.ylabel('CA (mol/m³)')
    plt.legend()
