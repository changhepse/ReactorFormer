...
def main(use_determined, args, info=None, context=None):
    ...
    metric, compare_metrics = get_metric(root, args.dataset) # nrmse, np.min
    
    model, ep_start, id_best, train_score, train_losses, train_time, embedder_stats_saved = load_state(use_determined, args, context, model, None, None, n_train, freq=args.validation_freq, test=True, dirname="best_")
    ...
    if ep_start == 0:
        save_state(use_determined, args, context, model, optimizer, scheduler, 0, n_train, train_score, train_losses, train_time, embedder_stats, "best_")
    ...
    model, ep_start, id_best, train_score, train_losses, train_time, embedder_stats_saved = load_state(use_determined, args, context, model, optimizer, scheduler, n_train, freq=args.validation_freq, dirname="best_")
    embedder_stats = embedder_stats if embedder_stats_saved is None else embedder_stats_saved

    if ep_start == args.epochs + args.predictor_epochs:
        print("\n------- Training Finished --------")
    ...
    if ep_start < args.epochs + args.predictor_epochs:
        ...
        for ep in range(ep_start, args.epochs + args.predictor_epochs):
            ...
            if ep % args.validation_freq == 0 or ep == args.epochs + args.predictor_epochs - 1:
                ...
                print("[train", "full epoch:" if ep >= args.predictor_epochs else "predictor epoch:", ep, "learning rate:", "%.6f" % optimizer.param_groups[0]['lr'], "] time elapsed:", "%.4f" % (train_time[-1]), "\ttrain loss:", "%.4f" % train_loss, "\tval loss:", "%.4f" % val_loss, "\tval score:", "%.4f" % val_score, "\tbest val score:", "%.4f" % compare_metrics(train_score))

                if use_determined:
                    id_current = save_state(use_determined, args, context, model, optimizer, scheduler, ep, n_train, train_score, train_losses, train_time, embedder_stats) # 返回ep
                    try:
                ...
                if compare_metrics(train_score) == val_score:
                    if not use_determined:
                        id_current = save_state(use_determined, args, context, model, optimizer, scheduler, ep, n_train, train_score, train_losses, train_time, embedder_stats, "best_") # 返回ep
                    id_best = id_current
...
def train_one_epoch(context, args, model, optimizer, scheduler, loader, loss, temp, ep=0):    
    ...
    for i, data in enumerate(loader):
        ...
        #print('idx_eq.shape', idx_eq.shape)
        
        if isinstance(x, list):
            #print('x is list type data')
            x, text_embeddings = x
        ...
        x = x.to(args.device) 
        out = model(x, text_embeddings=text_embeddings) #[32, 4, 64, 64] torch.cat([xfno, grid]) grid = torch.from_numpy(np.load(p + "/grid.npy"))
        ...
        idx_eq = idx_eq[0].item()

        if idx_eq == 0:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["CSTR1"], loader.dataset.std_dict["CSTR1"])
            input *= mask
            output *= mask
            l_res = pinn.cstr1(input, output)

        elif idx_eq == 1:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["CSTR2"], loader.dataset.std_dict["CSTR2"])
            input *= mask
            output *= mask
            l_res = pinn.cstr2(input, output)

        elif idx_eq == 2:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["BR1"], loader.dataset.std_dict["BR1"])
            input *= mask
            output *= mask
            l_res = pinn.br1(input, output)

        elif idx_eq == 3:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["BR2"], loader.dataset.std_dict["BR2"])
            input *= mask
            output *= mask
            l_res = pinn.br2(input, output)

        elif idx_eq == 4:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["PFR1"], loader.dataset.std_dict["PFR1"])
            input *= mask
            output *= mask
            l_res = pinn.pfr1(input, output)

        elif idx_eq == 5:
            x_m = x[:, :4, :, :] * mask
            input, output = denormalize(x_m, out, loader.dataset.mean_dict["PFR2"], loader.dataset.std_dict["PFR2"])
            input *= mask
            output *= mask
            l_res = pinn.pfr2(input, output)
        ...
        n_train += 1
        if n_train % 200 == 0:
            print(f'current batch train: {n_train}/{temp}')
            print('loss_batch_train:', "%.4f" % l.item())
            print('loss_batch_res:', "%.4f" % l_res.item())
    ...
    return train_loss / temp # temp = n_train


def eval_multiple(use_determined, context, root, args, model, loss, metric, optimizer, scheduler, n_train, id_best):
    print("\n------- Start Test --------")

    for pde_subset in ["CSTR1", "CSTR2", "CSTR3", "BR1", "BR2", "BR3", "PFR1", "PFR2", "PFR3", "ADR1", "ADR2"] :
        ...
        print("[test last]", "\ttime elapsed:", "%.4f" % (test_time_end - test_time_start), "\ttest loss:", "%.3e" % test_loss, "\ttest score:", "%.3e" % test_score)
            
        test_model, _, _, _, _, _, _ = load_state(use_determined, args, context, test_model, optimizer, scheduler, n_train, id_best, test=True, dirname="best_")
        test_time_start = default_timer()
        test_loss, test_score = evaluate(context, args, test_model, test_loader, loss, metric, n_test)
        test_time_end = default_timer()

        print("[test best]", "\ttime elapsed:", "%.4f" % (test_time_end - test_time_start), "\ttest loss:", "%.3e" % test_loss, "\ttest score:", "%.3e" % test_score)
            


def evaluate(context, args, model, loader, loss, metric, n_eval): # loss/metric = nrmse
    model.eval()
...
def save_state(use_determined, args, context, model, optimizer, scheduler, ep, n_train, train_score, train_losses, train_time, embedder_stats, dirname=""): # dirname=best_
    if not use_determined:
        path = 'results/'  + args.dataset +'/' + str(args.experiment_id) + "/" + dirname + str(args.seed)
        if not os.path.exists(path):
            os.makedirs(path)
        
        save_with_path(path, args, model, optimizer, scheduler, train_score, train_losses, train_time, embedder_stats)
        return ep
...
def save_with_path(path, args, model, optimizer, scheduler, train_score, train_losses, train_time, embedder_stats):
    ...
    np.save(os.path.join(path, 'train_time.npy'), train_time) # 训练时间应该保存
...
def load_state(use_determined, args, context, model, optimizer, scheduler, n_train, checkpoint_id=None, test=False, freq=1, dirname=""):
    ...
    train_time = np.load(os.path.join(path, 'train_time.npy'))
    ...
    return model, epochs, checkpoint_id, list(train_score), list(train_losses), list(train_time), embedder_stats # ep=ep_start, checkpoint_id=id_best



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='UPS')
    parser.add_argument('--config', type=str, default='./configs/base.yaml', help='config file name')
